...
### ▶ [Открыть в браузере](https://bratfedya.github.io/Gym_helper/)

Рабочая версия развёрнута на GitHub Pages. В Safari на iPhone её можно добавить на экран «Домой» — запустится в полноэкранном режиме, как обычное приложение.
...
```
.
├── index.html          # редирект на web/ — чтобы адрес Pages был коротким
...
│   ├── manifest.json   # PWA-манифест: standalone-режим, иконка, тёмный фон
...
### 1. Онлайн — ничего не нужно

**[bratfedya.github.io/Gym_helper](https://bratfedya.github.io/Gym_helper/)**

Развёрнуто на GitHub Pages из ветки `main` — деплой происходит автоматически при каждом пуше.

Чтобы получить приложение на iPhone без Xcode: открыть ссылку в **Safari** → «Поделиться» → **«На экран „Домой"»**. Благодаря `manifest.json` запустится в standalone-режиме, без адресной строки.
...
и открыть `http://localhost:8000`. Можно и просто открыть `web/index.html` двойным кликом.

### 3. Нативное приложение (Xcode)
...
> **Важно про данные.** У трёх способов разный origin (`https://`, `localhost`, `file://`), а значит и **разный `localStorage`**. Тренировки из нативного приложения не появятся в браузерной версии, и наоборот — это независимые копии. Перенести данные можно только через **Экспорт → Импорт** (JSON).

В браузере не работают кнопки **«Экспорт в файл»** и **«Импорт из файла»** — это нативный мост через Swift, в Safari его нет. Копирование в буфер и вставка JSON работают везде.
