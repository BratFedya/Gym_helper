...
Персональный трекер тренировок для iPhone. Ведёт библиотеку упражнений, собирает и генерирует тренировки, записывает рабочие веса и показывает прогресс.
...
- У каждой группы свой цвет и минималистичная SVG-иконка — упражнение видно с одного взгляда.
...
### Умный генератор
Не просто случайный выбор — алгоритм учитывает историю тренировок:
...
- **Распределяет нагрузку по мышцам**: мышцы, которые ты нагружал недавно, получают более низкий приоритет.
...
### Активная тренировка
- Отмечаешь каждое упражнение как выполненное по ходу занятия.
- Вводишь рабочий вес — поле **автоматически подставляет вес с прошлого раза**, чтобы не вспоминать.
- Есть режим «без веса» для упражнений с собственным весом.
- Невыполненные упражнения **не сохраняются** — в историю попадает только то, что ты реально сделал.
